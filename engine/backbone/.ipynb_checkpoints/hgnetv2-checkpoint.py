...
# 基于 "相邻层特征的差异" 生成注意力权重。网络会自动关注那些变化显著的特征，这些特征往往包含更重要的判别信息。
class TemporalDifferenceAttention(nn.Module):
    def __init__(self, channels, reduction=4):
        ...
        # 可学习的时间差权重
        self.td_weight = nn.Parameter(torch.tensor(0.5))

    def forward(self, features):
        # features: 包含输入特征和各层输出的列表 [x0, x1, x2, ..., xn]
        b, c, h, w = features[0].shape
        # 计算相邻特征的差异
        diffs = []
        for i in range(1, len(features)):
            # 确保特征尺寸一致
            prev_feat = F.adaptive_avg_pool2d(features[i-1], (h, w)) if features[i-1].shape[2:] != (h, w) else features[i-1]
            curr_feat = features[i]
            # 计算差分特征 (使用L1距离更适合捕捉变化)
            diff = torch.abs(curr_feat - prev_feat)
            diffs.append(diff)

        # 聚合所有差分特征
        diff_agg = torch.stack(diffs, dim=1).mean(dim=1)  # [B, C, H, W]

        # 生成差分注意力权重
        diff_att = self.fc(diff_agg)  # [B, C, H, W]

        # 应用注意力到原始特征 (结合原始特征和差分注意力)
        weighted_features = []
        for i, feat in enumerate(features):
            if i == 0:  # 输入特征使用初始权重
                weight = torch.sigmoid(self.td_weight)
                weighted = feat * weight
            else:  # 后续特征使用差分注意力
                # 动态调整权重：差异越大，权重越高
                weighted = feat * (1 - self.td_weight) + feat * diff_att * self.td_weight
...
class HG_Block(nn.Module):
    def __init__(
        ...
        self.tda = TemporalDifferenceAttention(
            mid_chs if layer_num > 0 else in_chs,
            reduction=4
        )
        # feature aggregation
    ...
    def forward(self, x):
        ...
        output[-3:-1] = self.tda(output[-3:-1])#[0]
